...
if (!class_exists('WPCheetahO')) {
    include_once(dirname(__FILE__) . '/lib/functions/cheetaho-ui.php');
    include_once(dirname(__FILE__) . '/lib/classes/cheetahoHelper.php');
    include_once(dirname(__FILE__) . '/lib/classes/cheetahoMetaHelper.php');
    ...
    class WPCheetahO
    {
        ...
        public function __construct()
        {
            ...
            include_once($plugin_dir_path . '/lib/cheetaho.php');
            ...
            add_action('wp_ajax_cheetaho_reset_all', array(
                &$this,
                'cheetahoMediaLibraryResetBatch'
            ));
            if (($this->cheetaho_settings != false && !empty($this->cheetaho_settings) && !empty($this->cheetaho_settings['auto_optimize'])) || ($this->cheetaho_settings != false && !isset($this->cheetaho_settings['auto_optimize']))) {

                add_action('add_attachment', array(
                    &$this,
                    'cheetahoUploaderCallback'
                ));
        ...
        {
            add_media_page('CheetahO Bulk Process', 'Bulk CheetahO', 'edit_others_posts', 'cheetaho-bulk',
                array(&$this, 'renderBulkProcess'));
        }
         ...
         */
        function cheetahoUploaderCallback($image_id)
        {
            try {
        ...
        public function cheetaho_ajax_callback()
        {
            ...
            if (wp_attachment_is_image($image_id)) {
                ...
                $data = array();
                if (empty($api_key)) {
                    $data['error'] = __('There is a problem with your credentials. Please check them in the CheetahO settings section and try again.',
                ...
                if ($status === false) {
                    $data['error'] = __('There is a problem with your cheetaho account. Maybe quota exceeded.',
                        'cheetaho-image-optimizer');
                    update_post_meta($image_id, '_cheetaho_size', $data);
                ...
                if (!isset($result['error']) && !isset($result['data']['error']) && isset($result['data']['destURL'])) {
                    ...
                    if (!isset($result['data']['originalSize']) || (int)$result['data']['originalSize'] == 0) {
                        echo json_encode(array(
                            'error' => array(
                                'message' => __('Could not optimize image. CheetahO can not optimize image. File size 0kb.',
                                    'cheetaho-image-optimizer')
                            )
                        ));
                        exit();
                    ...
                    if ($this->replace_new_image($local_image_path, $result['destURL'])) {
                        ...
                        echo json_encode(array(
                            'error' => array(
                                'message' => __('Could not overwrite original file. Please check your files permisions.',
                                    'cheetaho-image-optimizer')
                            )
                        ));
                    ...
                    if (isset($result['data']['error'])) {
                        $result = array();
                        $result['error'] = array();
                        $result['error']['message'] = __('Can not optimize image. Try later or contact CheetahO',
                            'cheetaho-image-optimizer');
                    }
        ...
        function optimize_thumbnails($image_data)
        {
            if (isset($image_data['file'])) {
                ...
                if (empty($image_id)) {
                    global $wpdb;
                    $post = $wpdb->get_row($wpdb->prepare("SELECT post_id FROM $wpdb->postmeta WHERE meta_value = %s LIMIT 1",
                        $image_data['file']));
        ...
        function optimizeImage($image_path, $type, $image_id, $throwException = false)
        {
            ...
            if (isset($settings['api_key']) && $settings['api_key'] != '') {
                ...
                if (isset($settings['optimize_retina']) && $settings['optimize_retina'] == 1 && isset($params['url']) && cheetahoMetaHelper::retinaName($params['url'])) {
                    $image_path = get_attached_file($image_id);
                ...
                if (isset($data['error']) && isset($data['error']['http_code']) && $data['error']['http_code'] == 403) {
                    ...
                    if ($status['data']['quota']['exceeded'] == false) {
                        $this->cheetahOUpdateNotice('quota', 0, 1);
                    } else {
                        $this->cheetahOUpdateNotice('quota', 0, 2);
                    }
                } else {
                    $status = $this->status;
        ...
        function cheetahoMediaLibraryReset()
        {
            $image_id = (int)$_POST['id'];
        ...
        <?php } ?>
            ...
            <small class="cheetahoReset" data-id="<?php echo $id; ?>"
                   title="<?php echo __('Removes Cheetaho metadata associated with this image',
                       'cheetaho-image-optimizer') ?>"> <?php echo __('Reset', 'cheetaho-image-optimizer') ?> </small>
...
EOD;
        ...
        function cheetahoMediaLibraryResetBatch()
        {
            $result = null;
        ...
        {
            $mylinks = array(
                '<a href="' . admin_url('options-general.php?page=cheetaho') . '">' . __("Settings",
                    "cheetaho-image-optimizer") . '</a>'
            );
        ...
        function validate_options_data($input)
        {
            $valid = array();
            $error = array();
            //$settings = get_option('_cheetaho_options'); check this
            ...
            $valid['resize'] = (isset($input['resize']) && $input['resize'] == 1) ? 1 : 0;
            ...
            if ($valid['resize'] == 1) {
                ...
                if ($valid['maxWidth'] == 0 || $valid['maxHeight'] == 0) {
                    $error[] = __('If you would like to resize image, the max height and max width fields must be filled',
                        'cheetaho-image-optimizer');
                }
            }
            ...
            foreach ($sizes as $size) {
                $valid['include_size_' . $size] = (isset($input['include_size_' . $size]) && $input['include_size_' . $size] == 1) ? 1 : 0;
            }
            ...
            if (empty($input['api_key'])) {
                ...
                if (isset($status['error'])) {
                    $error[] = __('Your API key is invalid. Check it here',
                            'cheetaho-image-optimizer') . ' http://app.cheetaho.com/admin/api-credentials';
                } else {
            ...
            if (!file_exists(CHEETAHO_BACKUP_FOLDER) && !@mkdir(CHEETAHO_BACKUP_FOLDER, 0777, true)) {
                $error[] = __("There is something preventing us to create a new folder for backing up your original files. Please make sure that folder",
                        'cheetaho-image-optimizer') . " <b>" . WP_CONTENT_DIR . '/' . CHEETAHO_UPLOADS_NAME . "</b> " . __("has the necessary write and read rights.",
                        'cheetaho-image-optimizer');
            }
    ...
    function getCheetahoUrl($action = 'options-general', $arg = array())
    {
        ...
        switch ($action) {
            ...
            case 'closeNotice':
                $url = wp_nonce_url(admin_url('admin-post.php?action=cheetahOCloseNotice&notice=' . $arg),
                    'cheetahOCloseNotice');
                break;
