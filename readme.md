...
> Sign up for your FREE CheetahO.com [account](https://app.cheetaho.com/) to get the API key and try out CheetahO image optimization tool. We suggest:
...
> * It is safe to test and use the plugin: all the original images are stored in a backup folder (optional feature, you can disable it)
...
- **I can not optimize the image. Got error "Requested file size too large". How fix this?**

    CheetahO lets to optimize an image up to 4 MB with free subscription plan. If you would like to optimize bigger files, you will need to take other plans from [here](https://app.cheetaho.com/billing/plans) 
...
- **My site is DEV mode on localhost. Can I use the plugin on a localhost installation?**

    You can not use plugin on a localhost. Your site is not public and our servers cann't download images to optimize.
...
Yes. You can filter images by the pattern. You can skip image by name or by the path. Useful if you want to skip images from remote storage (for example: /alicdn/)
